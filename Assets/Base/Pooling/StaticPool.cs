...
{
    public partial class StaticPool : Singleton<StaticPool>
    {
        ...
        protected void Spawn_Core<T>(int partitionIndex, out T spawnling, out bool isNew) where T : IPoolable
        {
            ...
            else
            {
                ...
                spawnling.Storage = storage;
                isNew = true;
            }
        }  

        internal void Spawn<T>(AmmoType type, Vector3 position, out T spawnling) where T : Ammo
        {
        ...
        internal void Spawn<T>(AmmoType type, out T spawnling) where T : Ammo
        {
            int partitionIndex = GetPartitionIndex(type);
        ...
        internal T Spawn<T>(AmmoType type, Vector3 position) where T : Ammo
        {
            Spawn(type, position, out T spawnling);
            return spawnling;
        }

        internal T Spawn<T>(AmmoType type) where T : Ammo
        {
        ...
        internal Ammo Spawn(AmmoType type, Vector3 position)
        {
            Spawn(type, position, out Ammo spawnling);
            //spawnling.ToggleOn();
            return spawnling;
        ...
        internal T Spawn<T>() where T: Ammo
        {
            return Spawn<T>(ammoTypeMap[typeof(T)]);
        }
    }

    public abstract class BaseProjectile : Ammo
    {
    ...
    public interface IPoolable : IIgnoreClean, IMono, IToggleable
    {
        Transform Storage { get; set; }

        void BackToPool();
    }

    public interface IIgnoreClean
    {
