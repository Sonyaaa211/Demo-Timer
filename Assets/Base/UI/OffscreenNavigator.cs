...
public class OffscreenNavigator : ObjectFollower<IOffscreenFollowable>
{
    ...
    internal override void SetFollow(IOffscreenFollowable follow)
    {
        gameObject.SetActive(true);
        target = follow.FollowTarget;
    ...
    bool HasNavigate()
    {
        ...
        planes = GeometryUtility.CalculateFrustumPlanes(SingletonRole.Cameraman.targetCamera);
        //planes = planes[0..4];
        if (GeometryUtility.TestPlanesAABB(planes, _targetCollider.bounds))
        {
            return false;
        }
        return Vector3.Distance(_targetCollider.transform.position, SingletonRole.Cameraman.targetCamera.transform.position) <= SingletonRole.Cameraman.targetCamera.farClipPlane;
    }
    ...
    protected override void StartFollow()
    {
        ...
        _backToCamVt = SingletonRole.Cameraman.transform.position - target.position;
        _backwardNormalVt = -SingletonRole.Cameraman.direction;
        ...
        _midPoint = Ref.Instance.WorldCanvas.transform.position;

        _midPort = (Vector2)SingletonRole.Cameraman.targetCamera.WorldToViewportPoint(_midPoint + _navDir) - OFFSET;
